...
export default function PolicyResult() {
  ...
  return (
    <div className="container page" style={{ maxWidth: 1080 }}>
        ...
        {(res) => (
          <div className="stack" style={{ gap: 20 }}>
            ...
            <div className="card card-pad" style={{ background: 'var(--teal-50)', borderColor: 'var(--teal-100)' }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--teal-700)', marginBottom: 6 }}>🧠 AI가 이해한 현재 상황</div>
              <div style={{ fontSize: 15 }}>{res.understood}</div>
            </div>
            ...
            {res.result === 'none' ? (
              ...
              <>
                <div className="muted" style={{ fontSize: 14.5 }}>회원님의 상황을 분석해 가장 적합한 순서로 추천했어요.</div>
...
function PolicyCard({ p, onFav, favActive }) {
  return (
    <div className="card card-pad card-hover">
      ...
      <p className="muted" style={{ marginBottom: 14 }}>{p.summary}</p>
      <div style={{ background: 'var(--teal-50)', borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
        <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--teal-700)', marginBottom: 6 }}>추천 이유</div>
        <ul>{p.reasons.map((r) => <li key={r} style={{ fontSize: 15, color: 'var(--ink-soft)', lineHeight: 1.7 }}>· {r}</li>)}</ul>
      </div>
      <div className="row" style={{ justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
        <span className="row muted" style={{ gap: 12, fontSize: 14.5 }}>
...
function NoResult({ reason }) {
  return (
    <div className="card card-pad">
      <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 8 }}>죄송해요. 입력하신 조건과 정확히 일치하는 정책을 찾지 못했어요.</div>
      <div style={{ background: 'var(--teal-50)', borderRadius: 10, padding: '14px 16px', margin: '8px 0 14px' }}>
        <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--teal-700)', marginBottom: 6 }}>AI 분석 결과</div>
        <div style={{ fontSize: 15 }}>{reason}</div>
        <div style={{ fontSize: 14, color: 'var(--ink-soft)', marginTop: 6 }}>조건을 수정하거나 추가 정보를 입력하시면 추천 가능한 정책을 다시 찾아드릴 수 있어요.</div>
      </div>
      <div style={{ fontSize: 14.5, fontWeight: 600, marginBottom: 8 }}>다른 도움 받기</div>
