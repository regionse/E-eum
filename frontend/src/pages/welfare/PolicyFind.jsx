...
const RECOMMENDATION_LOADING_STEPS = [
  {
    title: '상황 분석 중',
    description:
      '입력하신 생활 상태와 돌봄 상황을 이해하고 있어요.',
  },
  {
    title: '정책 검색 중',
    description:
      '회원님의 조건과 관련된 지원 정책을 찾고 있어요.',
  },
  {
    title: '적합도 판단 중',
    description:
      '정책별 자격조건과 지원 내용을 비교하고 있어요.',
  },
]
...
export default function PolicyFind() {
  ...
  useEffect(() => {
    ...
    const timerId = window.setInterval(() => {
      setLoadingStepIndex((previousIndex) => (
        (previousIndex + 1)
        % RECOMMENDATION_LOADING_STEPS.length
      ))
    }, 1600)
  ...
  const runRecommendation = async (
    ...
    try {
      ...
      if (response.status === 'invalid_input') {
        const retryText = response.retry_example
          ? `\n예: ${response.retry_example}`
          : ''

        setError(`${response.message}${retryText}`)
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <PolicyFormSection
                number="2"
                title="현재 생활"
                description="현재 상황과 가장 가까운 항목을 선택해주세요."
              >
                <SingleChoiceField
                  label="현재 주된 생활 상태"
                  value={form.current_life_status}
                  options={CURRENT_LIFE_STATUS_OPTIONS}
                  errors={validationErrors.current_life_status}
                  onChange={(value) => updateField('current_life_status', value)}
                />
              </PolicyFormSection>
            </div>
            ...
            >
              ...
              >
                <SingleChoiceField
                  ...
                  onChange={(value) => updateField('care_recipient', value)}
                />
                ...
                <SingleChoiceField
                  ...
                  onChange={(value) => updateField('care_duration', value)}
                />
                ...
                <SingleChoiceField
                  ...
                  onChange={(value) => updateField('daily_care_time', value)}
                />
              </div>
            ...
            >
              <SingleChoiceField
                ...
                onChange={(value) => updateField('financial_burden', value)}
              />
              ...
              >
                <MultiChoiceField
                  ...
                  onToggle={(value) => updateField(
                    'needed_support_types',
                    toggleMulti(form.needed_support_types, value, 'unknown')
                  )}
                />
              </div>
            </PolicyFormSection>
            ...
            >
              <textarea
                ...
                rows={5}
                maxLength={2000}
                placeholder="예) 조부모님을 돌보며 야간 아르바이트를 하고 있어요. 어떤 지원을 받아야 할지 잘 모르겠어요."
              ...
              >
                <span className="hint">
                  개인을 특정할 수 있는 이름이나 연락처는 적지 않아도 돼요.
                </span>
            ...
            >
              ...
              >
                위 섹션의 입력값은 정책 추천 중에만 사용하고 별도로 저장하지 않아요.
              </p>
            </div>
          </form>
        )}
...
function PolicyRecommendationLoading({
  stepIndex,
  onCancel,
}) {
  const currentStep =
    RECOMMENDATION_LOADING_STEPS[stepIndex]
  ...
  return (
    ...
    >
      ...
      >
        {RECOMMENDATION_LOADING_STEPS.map(
          (step, index) => (
            <span
              key={step.title}
              aria-hidden="true"
          ...
          >
            {options.map((option) => (
              <button
                ...
                type="button"
                className={`chip ${
                  selectedAnswers.includes(option) ? 'on' : ''
                }`}
