...
def pick_from_options(msg, options, cats=None):
    ...
    #  ★★★ 2026-08-11 — **후보 이름끼리 «포함 관계»면 긴 쪽만 남긴다.**
    #    실측(브라우저 시연) — 칩 [시각디자인·편집디자인·영상그래픽·**편집**·**영상편집**] 에서
    #      🧑「영상편집이요. 근데 저는 집 밖으로 오래 못 나가요.」
    #      → '편집' 도 '영상편집' 도 발화에 «들어 있다» → 복수선택으로 읽음
    #      → 호출부가 picked[0] = **'편집'** 을 카드로 만들었다(인쇄·출판).
    #    사용자는 칩 이름을 «정확히» 말했는데 다른 직업이 나온 것이다.
    #  ⇒ 짧은 이름이 «걸린 긴 이름 안»에 들어 있으면 그건 따로 고른 게 아니다.
    #    ⚠ 진짜 복수선택은 안 깨진다 — 「요양지원이랑 아이돌봄이요」는 서로 포함 관계가
    #      아니라 둘 다 남는다. 「편집이요」처럼 짧은 것만 말하면 긴 것이 애초에 안 걸린다.
    #    ⚠ 결과가 빌 수 없다 — 가장 긴 이름은 자기를 포함하는 다른 이름이 없어 항상 남는다.
    #  ★ 낱말을 더하는 게 아니라 «관계»를 보는 규칙이라, 앞으로 어떤 칩이 와도 같이 덮인다.
    if len(_named) > 1:
        _f = {o: re.sub(r'\s+', '', o) for o in _named}
        _named = [o for o in _named
                  if not any(_f[o] != _f[x] and _f[o] in _f[x] for x in _named)]
