...
def slide_a(prs):
    ...
    #  ⚠ 용어를 «빼지 않는다». 처음엔 「뜻으로/이름으로」로만 풀어 썼는데
    #    사용자 지적: 「엔트로피·벡터·FULLTEXT 이런 거 다 알려줘야 해. 용어는 다 써야지.
    #    그리고 해당하는 용어가 어떤 «역할»을 하는지도」
    #    ⇒ 용어 + 그 용어가 «하는 일» 을 한 줄씩 붙인다.
    LX, LW = 1.00, 7.60
    steps(s, LX, LW, 2.66, [
        ('① 슬롯을 이어 붙여 «질의»를 만듭니다', [
        ...
        ('② 두 갈래로 «따로» 찾습니다', [
            '벡터 검색 — Pinecone · gemini-embedding-2 · 3,072차원 · 코사인',
            '     뜻이 가까운 것을 찾습니다. 이름은 잘 못 잡습니다.',
            'FULLTEXT — MySQL · ngram 파서',
            '     글자가 일치하는 것을 찾습니다. 뜻은 못 봅니다.',
        ]),
        ('③ RRF 로 «순위»를 합칩니다  (k=60)', [
            'Reciprocal Rank Fusion — 점수가 아니라 «몇 등이었나»를 더합니다.',
            '코사인 0~1 과 FULLTEXT 0~수십은 단위가 달라 그냥 못 더합니다.',
            '양쪽 목록에 다 나온 후보가 위로 올라갑니다.',
        ]),
        ('④ 크로스인코더로 다시 줄 세웁니다', [
            'Jina reranker v3.5  (폴백 — Pinecone bge-reranker-v2-m3)',
            '임베딩은 질의와 후보를 «따로» 벡터로 만들어 거리를 잽니다.',
            '크로스인코더는 둘을 «붙여서 함께» 읽습니다. 정확한 대신 느립니다.',
        ]),
    ], pt_head=16, pt_body=12.5)
    ...
    node(2.70, 0.86, '“돌봄 어르신 사람”', '슬롯을 이어 붙인 «질의»', C_PANEL, C_PANLL)
    ...
    seg(s, CX + 2.40, 3.92, CX + 2.40, 4.14)
    node(4.14, 1.06, '벡터 검색', 'Pinecone · 코사인   |   뜻이 가까운 것',
         C_CARD, C_CARDL, w=4.30, x=CX - 4.55)
    node(4.14, 1.06, 'FULLTEXT', 'MySQL · ngram   |   글자가 맞는 것',
         C_CARD, C_CARDL, w=4.30, x=CX + 0.25)
    seg(s, CX - 2.40, 5.20, CX - 2.40, 5.46)
    seg(s, CX + 2.40, 5.20, CX + 2.40, 5.46)
    seg(s, CX - 2.40, 5.46, CX + 2.40, 5.46)
    seg(s, CX, 5.46, CX, 5.72)

    node(5.72, 0.92, 'RRF  (k=60)', '점수가 아니라 «순위»를 합친다', C_PANEL, C_PANLL)
    seg(s, CX, 6.64, CX, 6.90)
    node(6.90, 0.92, '크로스인코더 리랭커', 'Jina v3.5 — 질의+후보를 «붙여서» 읽는다',
         C_PANEL, C_PANLL)
    seg(s, CX, 7.82, CX, 8.02)

    box(s, CX - 3.40, 8.02, 6.80, 1.36, C_CARD, C_GREEN, 1.25)
    text(s, CX - 3.40, 8.20, 6.80, 0.30,
         [('후보 8개  (JOB_CAND_POOL)', 15, True, C_GREEN)], align=PP_ALIGN.CENTER)
    text(s, CX - 3.40, 8.58, 6.80, 0.60,
...
def slide_b(prs):
    ...
    #  값은 전부 itda_core.py 실측 상수 — CTOP_H 3679 · SPREAD_H 3670 · RR_LOW_CONF 3633
    LX, LW = 1.00, 7.60
    y = steps(s, LX, LW, 2.66, [
        ('1단  TOP%  —  1위 «묶음»이 얼마나 차지하나', [
            '후보를 NCS 중분류로 묶고, 1등 묶음의 점유율을 봅니다.',
            '0.90 미만 → 서로 «다른 동네»가 섞였다 → 방향 칩으로 되묻습니다.',
            '항목이 아니라 «묶음»으로 재는 게 핵심입니다 —',
            'CO₂·로봇·피복아크 용접은 항목으로는 31% 지만 묶으면 100% 입니다.',
        ]),
        ('2단  엔트로피  —  그 안에서 또 갈리나', [
            '한 동네로 좁혀졌어도 그 안에서 고르게 흩어져 있을 수 있습니다.',
            '0.70 이상 → 세부 칩으로 한 번 더 묻습니다.',
            '그 아래면 → 카드를 냅니다.',
        ]),
        ('확신이 낮으면  —  리랭커 최고점 < 0.13', [
            '중분류를 «얹되» 개별 직업은 안 버립니다 (계층 백오프).',
            '「보건복지 쪽입니다」만 주면 「요양보호사·다음 시험 8월 27일」보다 쓸모없습니다.',
        ]),
        ('카드 조립  —  없으면 «없다»고 말합니다', [
            '직업 + 자격증 ≤3 + K-MOOC 강좌 + 국비훈련 링크.',
            '자격증이 없는 직업이면 억지로 안 붙이고 내일배움카드를 안내합니다.',
        ]),
    ], pt_head=16, pt_body=12.5)
    text(s, LX, y + 0.06, LW, 0.30,
