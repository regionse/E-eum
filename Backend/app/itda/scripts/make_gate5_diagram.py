...
GATES = [
    ...
    ('④', '불법 신호인가',      '센다',    COUNT, '그대로 «통과»시키고 _abuse 만 +1'),
...
WHY = [
    ...
    #  ⚠ 사용자 질문: 「④가 불법 신호를 «센다»면서, 그 앞엔 LLM 을 안 부른다며?
    #    이 LLM 이 «막는다»는 건 무슨 소리야?」 — 주체가 둘인데 안 보였다.
    #      세는 것 = 코드(④)      막는 것 = LLM(통과한 뒤)
    #    ⇒ 「막는 건 «그 뒤에 오는» LLM」이라고 명시한다.
    ('④는 «막지 않고 통과»시킵니다', [
        '낱말로 감지만 하고 카운터를 +1 합니다.',
        '막는 건 «그 뒤에 오는» LLM 이 문맥으로 판단해서 합니다.',
        '낱말로 막으면 「마약류관리자」·「총포사」 같은',
        '실재 직업이 함께 걸립니다.',
    ]),
    ('그런데 왜 «세기»는 하나', [
        '예전엔 이게 없어서 레드팀 15턴에 카운터가 «0» 이었습니다.',
        'LLM 이 막아 사고는 없었지만, 그런 시도가 있었다는 걸',
        '«우리가 몰랐습니다». 모델이 바뀌면 그 방어가 사라지는데',
        '카운터가 0이라 «탐지조차» 안 됩니다.',
    ]),
]
...
def build(prs):
    ...
    #  끝
    #  ⚠ 아래 한 줄을 더 붙이면서 ye 를 0.30 → 0.20 으로 당겼다.
    #    안 당기면 마지막 줄 밑변이 11.30" 이 되어 슬라이드(11.25")를 넘는다.
    ye = ys[-1] + BH + 0.20
    seg(s, SPINE, ys[-1] + BH / 2, SPINE, ye + 0.25)
    #  ⚠ 예전 문구 「여기까지 전부 코드 · LLM 을 한 번도 안 부릅니다」가
    #    ④의 「LLM 이 막는다」와 «모순처럼» 보였다. 시점을 명시한다.
    ...
    text(s, RX + 0.40, ye + 0.60, RW - 0.40, 0.30,
         [('관문 여섯은 전부 코드 · 0원 · 약 1밀리초 — LLM 은 «그 뒤에» 한 번 부릅니다',
           12.5, False, C_MUTED)], align=PP_ALIGN.CENTER)
    return s
