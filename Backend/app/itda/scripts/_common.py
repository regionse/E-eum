# -*- coding: utf-8 -*-
"""잇다 배치 스크립트 공용 모듈 (2026-07-31 신설)

왜 만들었나
  배치 10개가 같은 코드를 각자 갖고 있었다. 실측:
      env 로더        7 파일 (공용 env.py 가 있는데도)
      DB 접속         8 파일
      Gemini 임베딩   3 파일
      Pinecone 준비   3 파일
      itda_sync_log   3 파일
  그래서 한 곳만 고치면 될 일을 매번 7~8곳에서 고쳤다. 2026-07-31 하루에만
  ① env 절대경로 제거 ② DB 접속 .env 화 ③ 콘솔 인코딩 — 세 번을 일괄 수정했다.
...
# ── Pinecone ────────────────────────────────────────────────────────
def index_name():
    """팀 통합 .env 스키마. 구 키(PINECONE_INDEX)도 읽는다 — 팀원이 아직 안 바꿨을 수 있다.
