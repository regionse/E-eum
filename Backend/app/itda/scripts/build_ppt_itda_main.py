...
def s7_measured(prs):
    ...
    #  ★ 2026-08-10 — 「약 1.3원 · LLM 1회」·「게이트는 의심스러울 때만」을 고쳤다.
    #    둘 다 «틀렸다». 5턴 대화를 돌려 모든 호출의 usage 를 합산해 다시 쟀다:
    #      턴당 2.3~3.5원(평균 2.79) · LLM 2~3회 · _harm_gate 는 매 턴 돈다(호출 추적으로 확인).
    cells = [
        ('34 / 34', '골든셋 회귀', '기대 답을 정해 둔 34문항 — 고칠 때마다 돌린다'),
        #  ★ 2026-08-10 (2차) — 「1.8턴」을 뺐다. 실측기록 §12 가 스스로 «대본이 협조적이라서»
        #    라고 자기부정하고, 동적 대화에선 §12-2 6턴 · §13-1 첫 카드 4턴이다.
        #    조건이 다른 숫자를 나란히 두면 물었을 때 답이 궁색해진다 ⇒ 동적 기준으로 바꾼다.
        ('3~4턴', '첫 결과 카드까지 (동적 대화)', '콕 집어 말하면 1턴 · 막연하면 되묻고 4~6턴'),
        #  ★ 단가는 게이트를 lite 로 되돌린 «뒤» 다시 쟀다(5턴 실측 2.53원, 되묻기 턴 2.38원).
    ...
    text(sl, 1.0, 9.80, 18.0, 0.9, [
        [('검사 스크립트 36개 · 실측 기록 2,196줄', 19, C_DEEP, True),
...
def s8_next(prs):
    ...
    cells = [
        #  ★★ 2026-08-10 (2차) — 「대화 접기」를 «앞으로 할 일»로 적었던 게 틀렸다.
        #    계층 요약(_facts + _summary + 최근 8턴)은 2026-08-06 에 «이미 들어가 있다».
        #    진짜 숙제는 그 옆 — 200턴 뒤 확인된 사실 30칸 중 22칸이 «사용자의 틀린 믿음»으로
        #    채워진다(②조현우 실측). 정보가 부족한 사용자일수록 초반에 오해부터 쏟아내는데,
        #    앞 12칸은 「안 변한다」는 전제로 «끝까지 안 버리는» 자리다.
        ('오래된 «오해» 걷어내기', ['긴 대화 요약은 이미 돈다. 문제는 그 «내용»이다 —',
                              '200턴 뒤 사실 30칸 중 22칸이 사용자의 틀린 믿음이었다.']),
        #  ★ 2026-08-10 — 「호출당 토큰이 절반」은 «틀렸다». 절을 실제로 갈라 토큰을 세보니
        #    공통부(하지말것·대상)가 양쪽에 들어가 총량이 8,685 로 «115%»가 됐다. 그래서
        #    「안 하기로 한 이유」로 바꿔 적는다 — 참관인에게 더 정직하고 더 강한 답이다.
        ('프롬프트 갈라 쓰기', ['상황별로 나누면 호출당 토큰이 준다 — 그런데 재보니 «총량은 15% 늘었다».',
                          '공통 규칙이 양쪽에 중복되고 호출이 순차라 지연도 2배. 그래서 안 한다.']),
...
def s9_wordlist(prs):
    """★ 2026-08-10 신설 — 「결과만 보여주고 과정을 뺐다」는 지적에 따라 추가.

    참관인·강사는 «중간»을 봐야 안다. 낱말표는 이 시스템에서 제일 큰 손품인데
    슬라이드에 한 글자도 없었다. 숫자는 전부 오늘 AST 로 다시 센 값이다.
    """
    sl = blank(prs)
    head(sl, 'PART 03 · 잇다 — 낱말표', '낱말표 1,023개 — 무엇을 어디에 쓰나')

    box(sl, 1.0, 2.65, 8.4, 8.1)
    text(sl, 1.45, 2.92, 7.6, 0.5, [[('두 갈래로 갈린다', 30, C_TITLE, True)]])
    text(sl, 1.45, 3.60, 7.6, 1.5, [
        [('안전층 ', 24, C_RED, True), ('188개', 34, C_RED, True)],
        [('위험 낱말 · 우회 표기 · 프롬프트 탈취', 17.5, C_EYE, False)],
    ], ls=1.25)
    text(sl, 1.45, 5.15, 7.6, 1.5, [
        [('대화 이해층 ', 24, C_DEEP, True), ('835개', 34, C_DEEP, True)],
        [('맞장구 · 거부 · 조사 · 정책어 — 한국어를 알아듣는 코드', 17.5, C_EYE, False)],
    ], ls=1.25)
    box(sl, 1.45, 6.75, 7.6, 3.7, fill=C_BG, line=C_BG_LN)
    text(sl, 1.75, 6.98, 7.0, 3.3, [
        [('★ 안전층의 절반은 «막는» 게 아니라 «살리는» 목록', 19, C_DEEP, True)],
        [('', 7, C_TXT, False)],
        [('「성폭력 상담사가 되고 싶어요」', 17.5, C_TXT, True)],
        [('「마약수사관은 어떻게 되나요」', 17.5, C_TXT, True)],
        [('— 위험 낱말이 들었지만 «진짜 진로 질문»이다', 16.5, C_EYE, False)],
        [('', 7, C_TXT, False)],
        [('예외 44개가 이걸 통과시킨다. 오차단 0/35.', 17, C_ACC, True)],
    ], ls=1.28)

    box(sl, 9.7, 2.65, 9.4, 8.1)
    text(sl, 10.1, 2.92, 8.6, 0.5, [[('어디서 어떻게 쓰이나', 30, C_TITLE, True)]])
    rows = [
        ('게이트 신호', '「죽고 싶어요」 → 낱말이 «신호» → LLM이 「누구 이야기인가」 판정'),
        ('검색어 오염 방지', '「ㅇㅋ」 → 이 말을 검색어로 «안 쓴다» → 쌓인 슬롯으로 찾는다'),
        ('슬롯 빼기', '「돌봄 말고」 → 누적 슬롯에서 제거 + 그 직업 배제'),
        ('조사 고르기', '「캐릭터콘텐츠제작」 + 받침 판정 → 「제작은」'),
        ('출력 가드', '「누구나 응시할 수 있어요」 → 그 문장만 떼어낸다'),
    ]
    #  ⚠ 다섯 칸(1.20×5 + 간격) + 출처 두 줄이 패널(높이 8.1) 안에 들어가야 한다.
    #    처음 판은 1.38 간격이라 마지막 줄이 패널 밖으로 나갔다(PNG 검수에서 잡음).
    y = 3.58
    for t, d in rows:
        box(sl, 10.1, y, 8.6, 1.20)
        text(sl, 10.4, y + 0.10, 3.1, 0.48, [[(t, 19.5, C_DEEP, True)]])
        text(sl, 10.4, y + 0.58, 8.0, 0.52, [[(d, 15.5, C_EYE, False)]])
        y += 1.30
    text(sl, 10.4, y + 0.10, 8.3, 1.0, [
        [('⚠ 출처는 공개 사전이 아니라 «실측 사고»다', 17.5, C_RED, True)],
        [('사고가 날 때마다 한 낱말씩 자랐다 — 재현성은 있지만', 15.5, C_EYE, False)],
        [('커버리지 보장은 없다. 그 한계도 숫자로 적어 뒀다(우회 29/32).', 15.5, C_EYE, False)],
    ], ls=1.26)


def s10_evidence(prs):
    """★ 2026-08-10 신설 — 판정의 «숫자»와 참고한 기법을 드러낸다.

    슬라이드가 「코드가 정한다」고만 말하고 무엇으로 정하는지를 안 보여줬다.
    """
    sl = blank(prs)
    head(sl, 'PART 03 · 잇다 — 근거', '판정은 «숫자»로 한다 — 그 숫자를 공개합니다')

    box(sl, 1.0, 2.65, 8.7, 8.1)
    text(sl, 1.45, 2.92, 7.9, 0.5, [[('착지 판정의 산수', 30, C_TITLE, True)]])
    text(sl, 1.45, 3.58, 7.9, 5.0, [
        [('세는 축은 «3개»뿐', 21, C_DEEP, True)],
        [('관심분야 · 활동유형 · 다루는대상', 19, C_ACC, True)],
        [('세부관심·제약·관심대분류는 안 센다 — 방향을 «가르지» 않으니까', 16, C_EYE, False)],
        [('', 9, C_TXT, False)],
        [('무게', 21, C_DEEP, True)],
        [('사용자가 말함 1.0  ·  코드가 채움 0.5', 18, C_TXT, False)],
        [('종류 — 원함 ×1.0  ·  해봤음 ×0.5  ·  못함 ×0.0', 18, C_TXT, False)],
        [('', 9, C_TXT, False)],
        [('문턱 2.0', 21, C_DEEP, True), ('   (12턴 넘으면 1.5로 완화)', 16, C_EYE, False)],
    ], ls=1.3)
    box(sl, 1.45, 8.75, 7.9, 1.72, fill=C_BG, line=C_BG_LN)
    text(sl, 1.78, 8.95, 7.3, 1.4, [
        [('「난 빵이 좋아요」 → 1.0 → 되묻기', 18, C_TXT, True)],
        [('「빵 만드는 일 하고 싶어요」 → 2.0 → 카드', 18, C_ACC, True)],
        [('무엇을(빵) + 어떻게(만들기)가 둘 다 있어야 한다', 15.5, C_EYE, False)],
    ], ls=1.3)

    box(sl, 10.0, 2.65, 9.1, 8.1)
    text(sl, 10.4, 2.92, 8.3, 0.5, [[('참고한 것 · 잰 것', 30, C_TITLE, True)]])
    text(sl, 10.4, 3.58, 8.3, 6.9, [
        [('설계에 쓴 기법', 21, C_DEEP, True)],
        [('RAG-Fusion — 질의 여러 개를 만들어 RRF로 합침', 17, C_TXT, False)],
        [('Critiquing — 「왜 아닌지」를 받아 다음을 찾는 설계', 17, C_TXT, False)],
        [('CRS 부정 피드백 — 거부한 것은 배제, 칩은 강등', 17, C_TXT, False)],
        [('계층 요약 — 오래된 대화를 접어 문맥을 유지', 17, C_TXT, False)],
        [('SAMHSA TIP 35 — 못 정하는 사람에겐 질문 대신 «보기»', 17, C_TXT, False)],
        [('', 9, C_TXT, False)],
        [('재보고 «버린» 것', 21, C_RED, True)],
        [('자기일관성(다수결) — 이득 대비 3배 비싸 껐다', 17, C_TXT, False)],
        [('직업 태그 3,282개 — 재현율 71%라 가중을 껐다', 17, C_TXT, False)],
        [('강점성향 슬롯 — 400발화에 0회라 뺐다', 17, C_TXT, False)],
        [('', 9, C_TXT, False)],
        [('논문 19편을 코드 주석에 근거로 달아 뒀습니다', 18, C_ACC, True)],
        [('실측 기록 2,196줄 · 검사 스크립트 36개', 18, C_ACC, True)],
    ], ls=1.26)
...
def main():
    ...
    s5_landing(prs)
    s6_answer(prs)
    s9_wordlist(prs)      # 낱말표 — 과정 공개 (2026-08-10 추가, 잰 것 «앞»에 둔다)
    s10_evidence(prs)     # 판정의 산수 + 참고 기법
    s7_measured(prs)
    s8_next(prs)
    prs.save(OUT)
