...
async def warmup():
    """Pinecone 연결을 **미리** 만들어 둔다 — 서버 기동 때 한 번 부른다 (2026-08-08 신설).

    왜 필요한가 (실측 2026-08-08)
      `_pc()` 가 클라이언트를 만드는 데 **5.28초**가 걸린다. 두 번째부터는 0.00초다.
      그래서 프로세스가 **처음 검색하는 턴**만 유독 느리다 —
        1~4턴(검색 없음)  1.3~1.6초
        5턴(첫 검색)      11.4초   ← 이 중 5.3초가 연결 생성
        6턴(두 번째 검색)  6.2초
      실사용에서는 «서버 재시작 후 첫 사용자»가 그 5.3초를 혼자 문다.
      챗봇에서 10초 넘는 침묵은 그 자체로 실패다.

    ⚠ 기동을 «막지 않는다» — 호출부가 create_task 로 던진다. 실패해도 조용히 넘긴다
      (키가 없거나 네트워크가 늦어도 서버는 떠야 한다. 첫 검색이 예전처럼 느릴 뿐이다).
    ⚠ 임베딩·리랭크는 데우지 않는다. 실측상 각각 0.7~0.8초로 연결 생성만큼 크지 않다.
    """
    import asyncio as _a
    try:
        await _a.to_thread(_pinecone)
        print('[itda] Pinecone 연결 예열 완료', flush=True)
    except Exception as e:                                  # noqa: BLE001
        print(f'[itda] Pinecone 예열 건너뜀 — {type(e).__name__}: {str(e)[:80]}', flush=True)
...
#  ★★★ 2026-08-10 — **k 를 60 → 1 로 내렸다. 재보고 바꾼 첫 값이다.**
#    그전까지 60 은 「RRF 표준값」이라는 이유뿐이었다 — 우리가 잰 값이 아니었다.
#    실측(checks/gate_ablation.py · 정답표 100건 · 원값 _gate_ablation.json):
#        k     @1   @3   @5   @8
#        0     68   84   92   95
#        1     63   84   92   95      ← 채택
#        60    53   72   79   86      ← 이전
#      k 가 작을수록 좋다가 «단조»다. 30 이상은 결과가 완전히 같다 —
#      1/(k+1) 과 1/(k+8) 의 차가 사라져 순위 정보가 통째로 버려진다는 뜻이다.
#    왜 60 이 우리한테 나빴나 — k 가 클수록 「몇 개 목록에 나왔나」가
#      「어디서 1등이냐」를 이긴다. Cormack 2009 의 60 은 «수십 개 시스템»을 합칠 때
#      나온 값이고, 우리는 «둘»(벡터·FULLTEXT)만 합친다. 표를 셀 재료가 없다.
#    ⚠ k=0 이 @1 은 5점 높지만 «채택하지 않았다» — @3·@5·@8 이 k=1 과 «완전히 같고»,
#      k=0 은 1위(1.000)가 2위(0.500)의 두 배라 한쪽 검색이 헛짚으면 그대로 1위가 된다.
#      우리는 상위 8개를 리랭커가 다시 줄 세우므로 «@8(풀에 들어왔나)»이 @1 보다 중요하다.
#      그 @8 이 86 → 95 로 +9 오른 것이 이 변경의 실익이다.
#    ⚠ 8/9 §14-3(질의 4개 조건)에서도 방향이 같았다 — k=1 @1=62 vs k=60 @1=47.
#    되돌리려면 .env 에 ITDA_RRF_K=60.
RRF_K = int(os.getenv('ITDA_RRF_K') or 1)


def _rrf(*ranked_id_lists, k=None):
    ...
    ⚠ k 기본값은 위 RRF_K 다(2026-08-10 실측으로 60 → 1). 아래 옛 서술은 60 시절 것이다.
    ...
    score = {}
    if k is None:
        k = RRF_K                       # 기본값을 «한 곳»에서 읽는다(위 RRF_K 주석)
...
async def match_jobs(db, query_text, top_k=6, min_score=0.0):
    ...
    #  ★★ 2026-08-06 — obj_detail(「누구를 대하는가」) 추가. obj_type='사람' 인 109개에만 있다.
    #    없으면 None 이고, 그 경우 가중에서 그냥 제외된다(itda_core 의 ATTR_LIFT 블록 참고).
    stmt = text("SELECT jc.job_code, jc.job_name, jc.job_mcls_name, jc.job_description, "
                "       COUNT(cj.cert_id), ja.act_type, ja.obj_type, ja.obj_detail "
                "FROM job_catalog jc "
                ...
                "GROUP BY jc.job_code, ja.act_type, ja.obj_type, ja.obj_detail").bindparams(
    ...
    for code in order:                           # RRF 합친 순서
        ...
        out.append({'job_code': str(r[0]), 'job_name': r[1], 'group': r[2],
                    ...
                    'obj_detail': r[7],                               # 「누구를」 (사람 대상 109개만)
